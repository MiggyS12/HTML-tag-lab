...
class Tests {
    constructor(context, testStatesEmitter, log, workspace) {
        this.handleChildProcess = (process) => tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        this.runTests = (tests, debuggerConfig) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            let testSuite = yield this.tests();
            for (const suiteOrTestId of tests) {
                const node = this.findNode(testSuite, suiteOrTestId);
                if (node) {
                    yield this.runNode(node, debuggerConfig);
                }
            }
        });
    ...
    loadTests() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            let output = yield this.initTests();
            ...
            let testSuite = yield this.getBaseTestSuite(tests);
    ...
    runNode(node, debuggerConfig) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (node.type === 'suite' && node.id === 'root') {
                ...
                let testOutput = yield this.runTestFile(`${node.file}`, debuggerConfig);
                testOutput = Tests.getJsonFromOutput(testOutput);
                this.log.debug('Parsing the below JSON:');
                this.log.debug(`${testOutput}`);
                let testMetadata = JSON.parse(testOutput);
                ...
                for (const child of node.children) {
                    yield this.runNode(child, debuggerConfig);
                }
                this.testStatesEmitter.fire({ type: 'suite', suite: node.id, state: 'completed' });
            }
                ...
                if (node.file !== undefined && node.line !== undefined) {
                    this.testStatesEmitter.fire({ type: 'test', test: node.id, state: 'running' });
                    let testOutput = yield this.runSingleTest(`${node.file}:${node.line + 1}`, debuggerConfig);
                    testOutput = Tests.getJsonFromOutput(testOutput);
                    this.log.debug('Parsing the below JSON:');
                    this.log.debug(`${testOutput}`);
                    let testMetadata = JSON.parse(testOutput);
